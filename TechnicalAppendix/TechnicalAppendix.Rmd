...
```{r, include = FALSE}
# library(rstudioapi)
# restartSession(command = "library(devtools)")
# 
...
modelData <- lockdown_data %>% dplyr::select(country, date, continent, gdp_capita, rolling_gcmr_retail, 
                                      rolling_gcmr_grocery, gcmr_retail_recreation, gcmr_grocery_pharmacy, days_in_lockdown, income, daily_confirmed, rolling_daily_confirmed) %>%  distinct()
...
modelData <- modelData %>% mutate(positive_rolling_gcmr_retail = -1*rolling_gcmr_retail, positive_rolling_gcmr_grocery = -1*rolling_gcmr_grocery)
```
...
ggplot(data = modelData, aes(x = days_in_lockdown, y = rolling_gcmr_retail)) + geom_line() + geom_line(aes(y = predict(initialModel1)), linetype = "dotdash", size = 0.8, color = "red")
ggplot(data = modelData, aes(x = days_in_lockdown, y = rolling_gcmr_retail)) + geom_line() + geom_line(aes(y = predict(initialModel2)), size = 0.8, color = "red")
```
...
# minModel1 <- glm(data = modelData, rolling_gcmr_retail ~ 1)
# maxModel1 <- glm(data = modelData, rolling_gcmr_retail ~ 1 + gdp_capita + average_temperature + national_lockdown_length + days_in_lockdown)




minModel1 <- glm.nb(data = filter(modelData, positive_rolling_gcmr_retail > 0, days_in_lockdown > 0), round(positive_rolling_gcmr_retail) ~ 1)
maxModel1 <- glm.nb(data = filter(modelData, positive_rolling_gcmr_retail > 0, days_in_lockdown > 0), round(positive_rolling_gcmr_retail) ~ 1 + gdp_capita + average_temperature + national_lockdown_length + days_in_lockdown + I(log(days_in_lockdown)))
#stepwise regression backwards, forwards and both directions
autoBack1 <- step(maxModel1, direction = "backward", scope = list("lower" = minModel1))
autoForward1 <- step(minModel1, direction = "forward", scope = list("upper" = maxModel1))
autoBoth1 <- step(minModel1, direction = "both", scope = list("lower" = minModel1, "upper" = maxModel1))  
summary(autoBack1)   
...
ggplot(data = filter(modelData, positive_rolling_gcmr_retail > 0, days_in_lockdown > 0), aes(x = days_in_lockdown, y = rolling_gcmr_retail, group = country, col = country)) + geom_line() + geom_line(aes(y = -exp(predict(autoBoth1))), linetype = "dotdash", size = 0.8)
```
...
```{r}
splineModelRetail <- glm(data = filter(modelData, gcmr_retail_recreation < 0, days_in_lockdown > 0), rolling_gcmr_retail ~ days_in_lockdown + national_lockdown_length + country + country:ns(days_in_lockdown, df = 5))
summary(splineModelRetail)
plot(splineModelRetail)
ggplot(data = filter(modelData, gcmr_retail_recreation < 0, days_in_lockdown > 0), aes(x = days_in_lockdown, y = rolling_gcmr_retail, group = country, col = country)) + geom_line() + geom_line(aes(y = predict(splineModelRetail)), linetype = "dotdash", size = 0.8)
```
...
minModel2 <- glm(data = filter(modelData, positive_rolling_gcmr_grocery > 0, days_in_lockdown > 0), round(positive_rolling_gcmr_grocery) ~ 1, family = poisson)
maxModel2 <- glm(data = filter(modelData, positive_rolling_gcmr_grocery > 0, days_in_lockdown > 0), round(positive_rolling_gcmr_grocery) ~ 1 + gdp_capita + average_temperature + national_lockdown_length + days_in_lockdown + I(days_in_lockdown^2) + I(days_in_lockdown^3), family = poisson)
#stepwise regression backwards, forwards and both directions

autoBack2 <- step(maxModel2, direction = "backward", scope = list("lower" = minModel2))
autoForward2 <- step(minModel2, direction = "forward", scope = list("upper" = maxModel2))
autoBoth2 <- step(minModel2, direction = "both", scope = list("lower" = minModel2, "upper" = maxModel2))  
summary(autoBack2)   
...
ggplot(data = filter(modelData, positive_rolling_gcmr_grocery > 0, days_in_lockdown > 0), aes(x = days_in_lockdown, y = rolling_gcmr_grocery, group = country, col = country)) + geom_line() + geom_line(aes(y = -exp(predict(autoBoth2))), linetype = "dotdash", size = 0.8)
#ggplot(data = modelData, aes(x = days_in_lockdown, y = rolling_gcmr_grocery)) + geom_line() + geom_line(aes(y = predict(autoBoth2)), linetype = "dotdash", size = 0.8, color = "red")
```
...
```{r}
splineModelGrocery <- glm(data = filter(modelData, gcmr_grocery_pharmacy < 0, days_in_lockdown > 0), rolling_gcmr_grocery ~ days_in_lockdown + national_lockdown_length + country + country:ns(days_in_lockdown, df = 5))
summary(splineModelGrocery)
plot(splineModelGrocery)
ggplot(data = filter(modelData, gcmr_grocery_pharmacy < 0, days_in_lockdown > 0), aes(x = days_in_lockdown, y = rolling_gcmr_grocery, group = country, col = country)) + geom_line() + geom_line(aes(y = predict(splineModelGrocery)), linetype = "dotdash", size = 0.8)
```
...
```{r}
minModel2 <- glm(data = filter(modelData, positive_rolling_gcmr_grocery > 0, days_in_lockdown > 0), round(positive_rolling_gcmr_grocery) ~ 1, family = poisson)
maxModel2 <- glm(data = filter(modelData, positive_rolling_gcmr_grocery > 0, days_in_lockdown > 0), round(positive_rolling_gcmr_grocery) ~ 1 + gdp_capita + average_temperature + national_lockdown_length + days_in_lockdown + I(days_in_lockdown^2) + I(days_in_lockdown^3) + country, family = poisson)
#stepwise regression backwards, forwards and both directions
...
#Model chooses average temp, lockdown length and days out of lockdown as variables. Days out of lockdown seems to be a very important contributor, with a small chance of rejection in a t-test. With all other things being equal, we would expect the gcmr activity to increase by 0.055176 with each consecutive day.
#Intuitively the only coefficient that does not make sense is the average temperature. Also has the largest effect so may need to reconsider. Additionally, we have not considered behaviour in different climes affecting gcmr differently.
...
na.omit(modelData) %>% ggplot(aes(y = rolling_gcmr_retail, x = days_out_lockdown, group = country))+geom_line()+geom_line(aes(y = predict(model1)), colour = 'red')

plot(model1)

# GOODNESS OF FIT
###
#QQ plot shows deviance on the tails, suggesting that this model may not be the best fit, only roughly approximating a normal distribution.
#
###
...
#
#Model uses all explanatory variables afforded to it, however the coefficients for lockdown length and days out of lockdown seem to be most important, owing to their low p value and - relatively - large contribution to the model. It must me remembered that what we are modelling, (daily confirmed cases per capita), can theoretically only take values between 1 and 0. In practice it only takes values lower than 0.002.
#So how would we interpret the days_out_lockdown coefficient for example? Think about a country that has been out of lockdown for x amount of days; its lockdown length and gdp will be the same on day x+1, so on the assumption that the average temperature has remained the same on days x and x+1, we would expect an increase in the daily confirmed cases per capita of 0.000001189 (1.189e-06). i.e. if a country has a population of 1  million, we would expect the amount of new daily cases on day x+1 to increase by 1.189 (1 person), relative to day x, (ceteris paribus).

#GDP per capita contributes a lot to the model with a coefficient of 
###
modelData <- na.omit(modelData)
...
regsubsetsOut <- regsubsets(dc_percapita ~ gdp_capita + days_out_lockdown + 
                              national_lockdown_length, data = modelData)
plot(regsubsetsOut, scale = "Cp")
...
na.omit(modelData) %>% ggplot(aes(y = dc_percapita, x = days_out_lockdown, group = country))+geom_line()+geom_line(aes(y = predict(lm1)), colour = 'red')
...
#Residuals seem to have variance that is fairly constant however there is an extremely long tail. This, in addition to the QQ plot,suggests that the GLM is slightly more appropriate to use as a model.
# When plotting residuals against fitted values of comparative model (lm1) we can see discernible patterns with both models however this looks stronger with stepwise regression model suggesting a better fit
