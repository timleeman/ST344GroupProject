...
knitr::opts_chunk$set(echo = TRUE, warnings = FALSE, fig.height = 15, fig.width= 15,fig.align='center')
```
...
plot_1<- filter(covidData1, national_lock %in% 1) %>%
  ...
  c('Coutries with National Lockdowns'='red',
...
### Final plots
```{r arranging the plots, fig.height = 20, fig.width= 15, warning = FALSE, message = FALSE}
...
```{r glm_for_gcmr_reatail_recreation, results = 'hide', warning = FALSE, error = FALSE}
...
```{r glm_for_gcmr_grocery_pharmacy}
minModel <- glm(data = na.omit(covidData1), 
...
```{r lockdown_length calculator, include=FALSE}
...
```{r functions_to_get_lockdown_data_and_bind, errors=FALSE, message=FALSE, warning=FALSE}
covidData<- covidData %>% 
  mutate(daily_confirmed = firstdiff(confirmed))
...
```{r calculating_the_r, errors=FALSE, message=FALSE, warning=FALSE}
...
```{r errors=FALSE, message=FALSE, warning=FALSE}
...
```{r exploring_Rdata, errors=FALSE, message=FALSE, warning=FALSE}
...
```{r functions_to_get_lockdown_data_and_bindb,}
selectData <- filter(covidData, country %in% chosenCountries)
...
```{r justification_for_first_modelb,}
...
#Model chooses average temp, lockdown length and days 
#out of lockdown as variables. Intuitively the only coefficient
#that does not make sense is the average temperature. 
#Also has the largest effect so may need to reconsider.
###
...
##Forward, backward and both directions stepwise 
#regression all choose the same model 
```
...
#Coefficients for lockdown length and days out of 
#lockdown seem to be most important
###
modelData <- na.omit(modelData)
...
anova(autoBoth)
##Forward, backward and both directions stepwise 
#regression all choose the same model which is encouraging
#similar
...
#fairly constand however there is an extremely long tail
...
#stronger with sepwise regression model suggesting better fit
