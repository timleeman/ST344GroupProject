...
```{r}
firstStageModelGrocery <- lm(gcmr_grocery_pharmacy ~ national_lock + confirmed_per_capita, data = covidData2)
summary(firstStageModelGrocery)
anova(firstStageModelGrocery)
```
...
lm(gcmr_retail_recreation ~ income + national_lock, data = filter(covidData3, date>as.Date("2020-08-15"))) %>% summary()
```


```{r glm for gcmr_reatail_recreation}
minModel <- glm(data = na.omit(covidData1), gcmr_retail_recreation ~ 1)
maxModel <- glm(data = na.omit(covidData1), gcmr_retail_recreation ~ national_lock + confirmed_per_capita + daily_deaths + deaths_per_capita + daily_confirmed + gdp_capita + factor(income))
...
maxModel <- glm(data = na.omit(covidData1), gcmr_grocery_pharmacy ~national_lock + confirmed_per_capita + daily_deaths + deaths_per_capita + daily_confirmed + gdp_capita)
