...
### Setting up the dataframe. This section adds data from Gapminder and Statista that we suscepted would be relevant to our gcmr models
```{r model_data,}
...
### This adds a new variable lockdown category to national_lockdowns based on proposed lengths
```{r}
lockdownCat <- c()
...
### Setting up the final data frame to be working with
```{r}
...
modelData$Place <- NULL #gets rid of the Place column, it was causing probblems with stepwise regression and is basically useless here
```
