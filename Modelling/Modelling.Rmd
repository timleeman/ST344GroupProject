...
```{r calculating_the_r,}
plot_calculate_r <- function(cou, startday, endday){
  startday <- lockdown_start_date(cou)
  endday <- lockdown_end_date(cou)
  key_data <- get_lockdown_data(cou) %>% dplyr::select(date, daily_confirmed, days_in_lockdown) %>%
    filter(date > startday & date < endday)
  daily_cases_model <- glm.nb(daily_confirmed ~ days_in_lockdown, data = key_data)
  key_data$pred_daily_cases <- predict(daily_cases_model)
  ggplot(key_data, aes(x= days_in_lockdown, y= log(daily_confirmed))) + geom_point() +
  geom_line(aes(y = pred_daily_cases))
  #return(coef(daily_cases_model)[2] %>% as.numeric())
}

plot_calculate_r("Jamaica")
