...
plot_country_lockdown_gcmr_retail <- function(cou){
  a <- modelData %>% filter(country %in% cou)
  ggplot(a, aes(x= days_in_lockdown, y= gcmr_retail_recreation, group = country)) + geom_line()
}
...
ggplot(filter(modelData,continent == "Europe"), aes(x=days_in_lockdown, y= gcmr_retail_recreation, group= country, col= country)) + geom_line()
+ geom_smooth(data = fortify(model1), aes(x = days_in_lockdown, y=gcmr_retail_recreation))
```
...
#gets rid of the Place column, it was causing probblems with stepwise regression and is almost all NA
modelData$Place <- NULL

modelData <- modelData %>% mutate(lockdownRatio = (days_in_lockdown / national_lockdown_length))
modelData <- filter(modelData, country != "El Salvador")
modelData <- filter(modelData, country != "Costa Rica")
...
minModel <- glm(data = modelData, gcmr_retail_recreation ~ 1, family = "gaussian")
maxModel <- glm(data = modelData, gcmr_retail_recreation ~ gdp_capita + days_in_lockdown + 
                                                            average_temperature + 
                                                            national_lockdown_length + country , family = "gaussian")
...
# 
# minpoissonModel <- glm(-1*gcmr_retail_recreation ~ 1, data = filter(modelData, gcmr_retail_recreation < 0), family = "poisson")
# maxpoissonModel <- glm(-1*gcmr_retail_recreation ~ days_in_lockdown + average_temperature + national_lockdown_length +
#                       factor(lockdownCategory) + gdp_capita + factor(income), data = filter(modelData, gcmr_retail_recreation < 0), family = "poisson")
# 
# autoPBack <- step(maxpoissonModel, direction = "backward", scope = list("lower" = minModel))
# autoPForward <- step(minpoissonModel, direction = "forward", scope = list("upper" = maxpoissonModel))
# autoPBoth <- step(minpoissonModel, direction = "both", scope = list("lower" = minpoissonModel, "upper" = maxpoissonModel))
# 
# summary(autoPBack)
# summary(autoPForward)
# summary(autoPBoth)
# 
# ratioModel <- glm(gcmr_retail_recreation ~ lockdownRatio, data = modelData)


ggplot(data = modelData, aes(x = days_in_lockdown, y= gcmr_retail_recreation)) + geom_line() + geom_smooth(data = fortify(autoBack), aes(x = days_in_lockdown, y = gcmr_retail_recreation))
ggplot(data = modelData, aes(x = days_in_lockdown, y= gcmr_retail_recreation)) + geom_line() + geom_smooth(data = fortify(autoForward), aes(x = days_in_lockdown, y = gcmr_retail_recreation))
ggplot(data = modelData, aes(x = days_in_lockdown, y= gcmr_retail_recreation)) + geom_line() + geom_smooth(data = fortify(autoBoth), aes(x = days_in_lockdown, y = gcmr_retail_recreation))


#plot_country_lockdown_gcmr_retail(c("France", "Italy")) + geom_line() + geom_smooth(data = filter(fortify(autoBack), country %in% c("France", "Italy")), aes(x = days_in_lockdown, y = gcmr_retail_recreation, group = country), method = NULL)
```
